...
function AudioModule:updateVolumeVisuals(i)
    ...
    setProp(mute, "CssClass", mute.Boolean and "icon-volume_mute" or "icon-volume_off")
    setProp(fader, "Color", mute.Boolean and "#CCCCCC" or "#0561A5")
end
...
function SystemAutomationController:registerEventHandlers()
    local eventMap = {
        btnSystemOnOff = function(ctl) if ctl.Boolean then self.powerModule:powerOn() else self.powerModule:powerOff() end end,
        btnSystemOn    = function() self.powerModule:powerOn() end,
        btnSystemOff   = function() self.powerModule:powerOff()
            self.state.motionGraceActive = true; setProp(controls.ledMotionGraceActive, "Boolean", true)
            self.timers.grace:Start(self.config.gracePeriod) end,
        btnAudioPrivacy = function(ctl) self.audioModule:setPrivacy(ctl.Boolean) end,
        roomName = function()
            local fmt = "[" .. controls.roomName.String .. "]"; self.roomName = fmt
            self:debugPrint("Room name updated: " .. fmt); self:publishNotification()
        end,
        ledMotionIn = function() self.motionModule:checkMotion() end,
        compCallSync = function() self:setCallSyncComponent() end,
        compSystemMute = function() self:setSystemMuteComponent() end,
        compACPR = function() self:setCamACPRComponent() end
    }
    for k, fn in pairs(eventMap) do bind(controls[k], fn) end
    -- Array controls
    bindArray(controls.btnVideoPrivacy, function(i, ctl) self.videoModule:setPrivacy(ctl.Boolean, i) end)
    bindArray(controls.knbVolumeFader, function(i, ctl) self.audioModule:setVolume(ctl.Position, i) end)
    bindArray(controls.btnVolumeMute, function(i, ctl) self.audioModule:setMute(ctl.Boolean, i) end)
    bindArray(controls.btnVolumeUp, function(i, ctl) self.audioModule:setVolumeUpDown("up", ctl.Boolean, i) end)
    bindArray(controls.btnVolumeDn, function(i, ctl) self.audioModule:setVolumeUpDown("down", ctl.Boolean, i) end)
    forEach(controls.compVideoBridge, function(i, ctrl)
        bind(ctrl, function() self:setVideoBridgeComponent(i) end) end)
    forEach(controls.compGains, function(i, ctrl)
        bind(ctrl, function() self:setGainComponent(i) end) end)
    forEach(controls.devDisplays, function(i, ctrl)
        bind(ctrl, function() self:setDisplayComponent(i) end) end)
    forEach(controls.typeGain, function(i, ctrl)
        if i > 1 then bind(ctrl, function(ctl)
...
-- Factory function to create the system controller
