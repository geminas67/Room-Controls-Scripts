...
function UCIController:initializeRoomControlsComponent()
    ...
    if success and component then
        ...
        print("Warning: Room Controls Component not found - using direct control method")
        self.roomControlsComponent = nil
    end
end
    ...
    if self.roomControlsComponent then
        local success, result = pcall(function()
            if isPoweringOn then
                return self.roomControlsComponent["warmupTime"] and self.roomControlsComponent["warmupTime"].Integer or 10 -- default 10 seconds, change to "WarmupTime" to use component value set by user
            else
                return self.roomControlsComponent["cooldownTime"] and self.roomControlsComponent["cooldownTime"].Integer or 5 -- default 5 seconds, change to "CooldownTime" to use component value set by user
            end
        end)
...
function UCIController:setupEventHandlers()
    ...
    Controls.pinLEDACPRActive.EventHandler = function(ctl)
        self:updateACPRState(ctl.Boolean)
    end
...
function UCIController:updateACPRState(isActive)
    if isActive then
...
-- Modify showLayer to use state information
...
myUCI = createUCIControllerWithErrorHandling(
    "UCI MPR(005)",  -- Use exact UCI name
