--[[
  Sony Bravia DisplayController — Q-SYS control script for Sony Bravia displays
...
local componentTypes = {
    displays = "%PLUGIN%_C76AD0FA-D707-4bb4-991E-D70D77AC1FC4_%FP%_b1533bca5f02f791538ad7a9a5ed9903",
...
local function validateControls()
    ...
    if #missing > 0 then
        print("ERROR: Sony Bravia DisplayController validation failed — missing: " .. table.concat(missing, ", "))
        return false
    end
    return true
end
...
local function displayPowerAll(powerOn)
    debugPrint("Power all displays → " .. (powerOn and "ON" or "OFF") .. " (Source: UCI)")
    for _, display in pairs(components.displays) do
        if display then
...
local function displayPowerSingle(index, powerOn)
    ...
    local control = powerOn and displayControls.powerOn or displayControls.powerOff
    safeComponentAccess(display, control, "trigger")
    debugPrint("Display " .. index .. " power → " .. (powerOn and "ON" or "OFF") .. " (Source: UCI)")
end

local function displaySetInputAll(input)
    debugPrint("All displays input → " .. input .. " (Source: UCI)")
    for _, display in pairs(components.displays) do
        if display then
...
local function displaySetInputSingle(index, input)
    ...
    if display[displayControls.inputSelectComboBox] then
        safeComponentAccess(display, displayControls.inputSelectComboBox, "setString", input)
        debugPrint("Display " .. index .. " input → " .. input .. " (ComboBox) (Source: UCI)")
    else
        local buttonNumber = getInputButtonNumber(input)
        if buttonNumber then
            local buttonName = displayControls.inputSelectButtons .. buttonNumber
            safeComponentAccess(display, buttonName, "trigger")
            debugPrint("Display " .. index .. " input → " .. input .. " (buttons) (Source: UCI)")
        end
    end
end
...
local ok, err = pcall(function()
    print("Initializing Sony Bravia DisplayController for " .. const.roomName .. "...")
...
if ok then
    print("✓ Sony Bravia DisplayController initialized — " .. const.roomName)
    ...
    mySonyBraviaDisplayController = SonyBraviaDisplayController
    SonyBraviaDisplayControllerInstance = SonyBraviaDisplayController
else
    ...
    SonyBraviaDisplayController = nil
    mySonyBraviaDisplayController = nil
    SonyBraviaDisplayControllerInstance = nil
end
