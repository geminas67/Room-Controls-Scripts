--[[
  ...
  2025-06-23
  Version: 1.3 (Universal Video Switcher Integration)
      Firmware Req: 10.0
    Notes:
...
function VideoSwitcherIntegration.new()
    ...
    function self:initialize()
        ...
        -- self:setupUCIButtonMonitoring() -- Disabled - using direct event handlers instead
        
        self.isEnabled = true
        self:debugPrint("Video Switcher Integration initialized successfully")
        self:debugPrint("Using direct event handlers for buttons: btnNav07, btnNav08, btnNav09")
        self:debugPrint("Mapping: btnNav07→" .. self.uciToInputMapping[7] .. ", btnNav08→" .. self.uciToInputMapping[8] .. ", btnNav09→" .. self.uciToInputMapping[9])
        return true
    end
    
    -- Re-initialize video switcher integration (for delayed initialization)
    function self:reinitialize()
        self:debugPrint("Re-initializing Video Switcher Integration")
    ...
    function self:setupUCIButtonMonitoring()
        if not self.isEnabled then 
            self:debugPrint("⚠ Video switcher not enabled - skipping button monitoring")
            return 
        end
        ...
        self.monitoringTimer.EventHandler = function()
            -- Add periodic debug output to confirm timer is running
            self:debugPrint("Timer tick - checking " .. table.getn(self.uciToInputMapping) .. " buttons")
            ...
            for uciButton, inputNumber in pairs(self.uciToInputMapping) do
                ...
                if Controls[buttonName] then
                    ...
                    -- Debug: Log button states periodically (more frequent)
                    if currentState then
                        self:debugPrint("Monitoring: " .. buttonName .. " = " .. tostring(currentState) .. " (prev: " .. tostring(previousState) .. ")")
                    end
                    
                    -- Check if button state changed to true
                    if currentState and not previousState then
                        self:debugPrint("Button " .. buttonName .. " pressed - triggering video switcher")
                    ...
                    self:debugPrint("⚠ Button " .. buttonName .. " not found in Controls")
                end
            end
        end
        
        -- Start the monitoring timer with repeat
        self.monitoringTimer:Start(0.1, true) -- Check every 100ms, repeat = true
        
        -- Debug: List monitored buttons
        local buttonList = {}
        for uciButton, _ in pairs(self.uciToInputMapping) do
            table.insert(buttonList, "btnNav" .. string.format("%02d", uciButton))
        end
        self:debugPrint("UCI button monitoring started for buttons: " .. table.concat(buttonList, ", "))
        
        -- Add immediate test of button states
        self:debugPrint("Initial button states:")
        for uciButton, _ in pairs(self.uciToInputMapping) do
            local buttonName = "btnNav" .. string.format("%02d", uciButton)
            if Controls[buttonName] then
                self:debugPrint("  " .. buttonName .. " = " .. tostring(Controls[buttonName].Boolean))
            end
        end
    end
        ...
        ctl.EventHandler = function()
            ...
            if self.videoSwitcher and self.videoSwitcher.isEnabled then
                ...
                if inputNumber then
                    ...
                    self.videoSwitcher:switchToInput(inputNumber, i)
                end
            end
        end
    end
...
function checkAndReinitializeVideoSwitcher()
    if myUCI and myUCI.videoSwitcher then
        ...
        if Controls.devNV32 then
            local devNV32Value = Controls.devNV32.String or ""
            ...
            if devNV32Value ~= "" then
                ...
                print("devNV32 is empty")
            end
        else
            print("devNV32 variable not found")
        end
