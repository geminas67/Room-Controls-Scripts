...
function CameraPresetController:registerEventHandlers()
    ...
    -- Avoiding bind() wrapper function to prevent intermittent failures
    ...
    if controls.devCams then
        ...
        self:debug("Camera selection event handler registered")
    end
    ...
    if controls.compcamRouter then
        ...
        self:debug("Router selection event handler registered")
    end
    
    -- Configuration control handlers
...
function CameraPresetController:handlePresetSave(presetIndex)
    if self.cameraModule:savePreset(presetIndex) then
        -- Flash the LED to indicate successful save
...
function CameraPresetController:setupCameraChoices(cameraNames)
    ...
    -- Check if devCams is an array or single control
    local devCamsControls = getControlArray(controls.devCams)
    
    if #devCamsControls == 0 then
        self:error("No devCams controls found in array")
        return
    end
    
    -- Set camera choices in the UI
    for _, devCamsControl in ipairs(devCamsControls) do
        if devCamsControl then
            if setProp(devCamsControl, "Choices", cameraNames) then
                self:debug(string.format("Camera choices populated: %d cameras for control", #cameraNames))
            else
                self:debug("Camera choices already set or control unavailable")
            end
        else
            self:error("Invalid devCams control in array")
        end
    end
    
    -- Verify choices were set
    local firstControl = devCamsControls[1]
    if firstControl and firstControl.Choices then
        self:debug(string.format("Verified choices count: %d (expected: %d)", 
            #firstControl.Choices, #cameraNames))
        for i, choice in ipairs(firstControl.Choices) do
            self:debug(string.format("  Choice[%d]: %s", i, choice))
        end
    else
        self:error("Failed to verify camera choices - control or Choices property not available")
    end
    
    controls.txtJSONStorage.IsDisabled = true
    
    -- Set default camera selection with fallback logic
    if #cameraNames > 0 and firstControl then
        ...
        for i, camName in ipairs(cameraNames) do
            if camName == self.config.defaultCamera then
                firstControl.String = camName
                firstControl.Value = i
                defaultSet = true
        ...
        if not defaultSet then
            firstControl.String = cameraNames[1]
            firstControl.Value = 1
        ...
        if #cameraNames == 0 then
            ...
            self:error("Camera control not available for default selection")
        end
    end
end
