--[[
  UCIController class - Enhanced Version (External Controller Notification) with Universal Video Switcher Integration
  Author: Nikolas Smith, Q-SYS
  2025-06-23
  Version: 1.4 (Performance Optimized String Concatenation)
      Firmware Req: 10.0
    Notes:
...
-- Safe Controls access helper function (for optional controls only)
...
function VideoSwitcherIntegration.new()
    ...
    -- Set up UCI button monitoring (direct Controls access like Brookgreen version)
...
function UCIController:callActivePopup()
    local callActiveState = Controls.pinCallActive and Controls.pinCallActive.Boolean or false
...
function UCIController:showPresetSavedSublayer()
    self:updatePresetSavedState(Controls.pinLEDPresetSaved and Controls.pinLEDPresetSaved.Boolean or false)
end

function UCIController:showHDMISublayer()
    self:updateHDMI01State(Controls.pinLEDHDMI01Connect and Controls.pinLEDHDMI01Connect.Boolean or false)
end

function UCIController:showLaptopHelpSublayer()
    self:updateLaptopHelpState(Controls.btnHelpLaptop and Controls.btnHelpLaptop.Boolean or false)
end

function UCIController:showACPRSublayer()
    self:updateACPRBypassState(Controls.pinLEDACPRBypassActive and Controls.pinLEDACPRBypassActive.Boolean or false)
end

function UCIController:showPCHelpSublayer()
    self:updatePCHelpState(Controls.btnHelpPC and Controls.btnHelpPC.Boolean or false)
end

function UCIController:showWirelessHelpSublayer()
    self:updateWirelessHelpState(Controls.btnHelpWireless and Controls.btnHelpWireless.Boolean or false)
end

function UCIController:showRoutingHelpSublayer()
    self:updateRoutingHelpState(Controls.btnHelpRouting and Controls.btnHelpRouting.Boolean or false)
end

function UCIController:showDialerHelpSublayer()
    self:updateDialerHelpState(Controls.btnHelpDialer and Controls.btnHelpDialer.Boolean or false)
end

function UCIController:showStreamMusicHelpSublayer()
    self:updateStreamMusicHelpState(Controls.btnHelpStreamMusic and Controls.btnHelpStreamMusic.Boolean or false)
end
...
function UCIController:interlock()
    for i = 1, 12 do
        local buttonName = getButtonName(i)
        if Controls[buttonName] then
    ...
    if btnIndex then
        local buttonName = getButtonName(btnIndex)
        if Controls[buttonName] then
        ...
        if ctl then
            ctl.EventHandler = function()
                ...
                if self.videoSwitcher and self.videoSwitcher.isEnabled then
                    local inputNumber = self.videoSwitcher.uciToInputMapping[i]
                    if inputNumber then
                        print("=== UCI Button " .. i .. " pressed - triggering video switcher to input " .. inputNumber .. " ===")
                        local success = self.videoSwitcher:switchToInput(inputNumber, i)
                        if success then
                            print("✓ Video switcher command sent successfully")
                        else
                            print("✗ Video switcher command failed")
                        end
                        print("=== End Video Switcher Trigger ===")
                    end
                end
            end
        else
...
-- Delayed check for devNV32 variable (in case it's set after initialization)
Timer.CallAfter(function()
    checkAndReinitializeVideoSwitcher()
end, 5)
...
function testVideoSwitcherComponent()
    if myUCI and myUCI.videoSwitcher then
        ...
        if status.componentValid and myUCI.videoSwitcher.switcherComponent then
            ...
            print("Routing Method: " .. tostring(routingMethod))
            
            -- Test if the routing method property exists
            if component[routingMethod] then
                print("✓ Routing method property exists")
                print("Current value: " .. tostring(component[routingMethod].Value))
            else
                print("✗ Routing method property not found")
            end
        else
            print("✗ Component not valid")
        end
...
function testButtonPress(buttonNumber)
    if myUCI and myUCI.videoSwitcher then
        local buttonName = getButtonName(buttonNumber)
        print("=== Testing Button Press " .. buttonName .. " ===")
        if Controls[buttonName] then
...
-- Test function to manually trigger button monitoring
function testButtonMonitoringManual()
    if myUCI and myUCI.videoSwitcher then
        print("=== Manual Button Monitoring Test ===")
        
        -- Manually call the timer event handler
        if myUCI.videoSwitcher.monitoringTimer and myUCI.videoSwitcher.monitoringTimer.EventHandler then
            print("Manually triggering timer event handler...")
            myUCI.videoSwitcher.monitoringTimer.EventHandler()
            print("Timer event handler executed")
        else
            print("✗ Timer or event handler not available")
        end
...
-- Test function to check button monitoring status
function testButtonMonitoring()
    if myUCI and myUCI.videoSwitcher then
        print("=== Testing Button Monitoring ===")
        local status = myUCI:getVideoSwitcherStatus()
        print("Video Switcher Enabled: " .. tostring(status.enabled))
        print("Component Valid: " .. tostring(status.componentValid))
        
        -- Check if buttons exist
        for uciButton, inputNumber in pairs(status.mapping or {}) do
            local buttonName = getButtonName(uciButton)
            if Controls[buttonName] then
                print("✓ " .. buttonName .. " exists, current state: " .. tostring(Controls[buttonName].Boolean))
            else
                print("✗ " .. buttonName .. " not found")
            end
        end
        
        -- Test direct video switcher call
        print("Testing direct video switcher call...")
        local success = myUCI:switchVideoInput(5)
        print("Direct call result: " .. tostring(success))
        
        print("=== End Button Monitoring Test ===")
    else
        print("UCI Controller or Video Switcher not available")
    end
end
...
-- Function to check devNV32 variable and reinitialize if needed
function checkAndReinitializeVideoSwitcher()
    if myUCI and myUCI.videoSwitcher then
        print("=== Checking devNV32 Variable ===")
        
        -- Check if devNV32 variable exists and has a value
        local devNV32Value = Controls.devNV32 and Controls.devNV32.String or ""
        print("devNV32 = '" .. devNV32Value .. "'")
        
        if devNV32Value ~= "" then
            print("devNV32 is set - attempting to reinitialize video switcher")
            local success = myUCI.videoSwitcher:reinitialize()
            if success then
                print("✓ Video switcher reinitialized successfully")
                printVideoSwitcherStatus()
            else
                print("✗ Failed to reinitialize video switcher")
            end
        else
            print("devNV32 is empty or not found")
        end
        
        print("=== End devNV32 Check ===")
    else
        print("UCI Controller not available")
    end
end
...
print("✓ Test functions available: testVideoSwitcherInput(inputNumber) and testButtonPress(buttonNumber)")
