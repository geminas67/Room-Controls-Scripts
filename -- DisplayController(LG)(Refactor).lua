...
local function validateControls()
    ...
    print("LGDisplayController validation passed")
    return true
end
...
-------------------[ Class Definition ]-------------------
LGDisplayController = {}
LGDisplayController.__index = LGDisplayController

function LGDisplayController.new(roomName, config)
    ...
    local self = setmetatable({}, LGDisplayController)
...
-----------------------------[ Debug Helper ]-----------------------------
function LGDisplayController:debugPrint(str)
    if self.debugging then print("["..self.roomName.." Debug] "..str) end
end

-----------------------------[ Dynamic Timer Configuration ]-----------------------------
function LGDisplayController:updateTimerConfigFromComponent()
...
function LGDisplayController:getTimerConfig(isWarmup)
    self:updateTimerConfigFromComponent()
    return isWarmup and self.timerConfig.warmupTime or self.timerConfig.cooldownTime
end

-----------------------------[ Input Button Mapping ]-----------------------------
function LGDisplayController:getInputButtonNumber(input)
...
function LGDisplayController:safeComponentAccess(component, control, action, value)
...
-----------------------------[ Display Module ]-----------------------------
function LGDisplayController:initDisplayModule()
    local selfRef = self
    self.displayModule = {
...
-----------------------------[ Power Module ]-----------------------------
function LGDisplayController:initPowerModule()
    local selfRef = self
    self.powerModule = {
...
function LGDisplayController:setComponent(ctrl, componentType)
...
function LGDisplayController:setComponentInvalid(componentType)
    self.components.invalid[componentType] = true
    self:checkStatus()
end

function LGDisplayController:setComponentValid(componentType)
    self.components.invalid[componentType] = false
    self:checkStatus()
end

function LGDisplayController:checkStatus()
...
-----------------------------[ Component Setup ]-----------------------------
function LGDisplayController:setupDisplayComponents()
...
function LGDisplayController:setRoomControlsComponent()
...
function LGDisplayController:setDisplayComponent(index)
...
function LGDisplayController:setupDisplayEvents(index)
...
function LGDisplayController:getComponentNames()
    local namesTable = {
        DisplayNames = {},
        RoomControlsNames = {},
    }
...
function LGDisplayController:updateRoomNameFromComponent()
    if self.components.compRoomControls then
...
function LGDisplayController:registerTimerHandlers()
...
function LGDisplayController:registerEventHandlers()
...
-----------------------------[ Initialization ]-----------------------------
function LGDisplayController:funcInit()
    self:debugPrint("Starting LG DisplayController initialization...")
    ...
    self:debugPrint("LG DisplayController Initialized with " .. 
                   self.displayModule.getDisplayCount() .. " displays")
end

-----------------------------[ Cleanup ]-----------------------------
function LGDisplayController:cleanup()
...
-----------------------------[ Factory Function ]-----------------------------
local function createLGDisplayController(roomName, config)
    local defaultRoomName = roomName or "[LG Display]"
    print("Creating LG DisplayController for: " .. defaultRoomName)
    
    local success, controller = pcall(function()
        local instance = LGDisplayController.new(defaultRoomName, config)
    ...
    if not success then
        print("ERROR: Failed to create LG DisplayController: " .. tostring(controller))
        return nil
    end
    
    print("Successfully created and initialized LG DisplayController for " .. defaultRoomName)
    return controller
end

-----------------------------[ Global Export and Instance Creation ]-----------------------------
-- Export the class globally for external access
LGDisplayController = LGDisplayController
...
myLGDisplayController = createLGDisplayController(roomName, config)

if myLGDisplayController then
    print("SUCCESS: LG DisplayController created and initialized!")
    print("Room: " .. roomName)
    print("Display count: " .. myLGDisplayController.displayModule.getDisplayCount())
    
    -- Export instance globally for external access
    LGDisplayControllerInstance = myLGDisplayController
else
    print("ERROR: Failed to create LG DisplayController!")
end
...
--[[
  ...
  ✓ Removed wall-related controls/functions - Power and Input only (v2.1)
]] 